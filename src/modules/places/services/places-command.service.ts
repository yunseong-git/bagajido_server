import {
  ...
  export class PlacesCommandService {
    ...
    async createCategory(
      ...
      const existingCategory =
        await this.placesRepository
          .findCategoryByKey(dto.key);
  
      if (existingCategory) {
        throw new ConflictException(
          'PLACE_CATEGORY_KEY_ALREADY_EXISTS',
        );
      }
    ...
    private async assertPlaceExists(
      id: string,
    ) {
      const place =
        await this.placesRepository
    ...
    private handleUniqueConstraint(
      ...
      if (
        ...
        throw new ConflictException(
          'PLACE_EXTERNAL_SOURCE_ALREADY_EXISTS',
        );
      }
